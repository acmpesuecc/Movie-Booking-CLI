...
void InputDetails()
{
    ...
    getchar();
    fprintf(fp, "%s, %s, %s,",dynamic_array[count].name,dynamic_array[count].email,dynamic_array[count].mobile);
    count++;
    fclose(fp);
}
    ...
    for (int i = 0; i < count; i++)
    {
        if (strstr(dynamic_array[i].name, search) != NULL)
        {
            ...
            if (n == 5)
                ...
                char line[100];
                char *token;
                const char *target_name = search;
                
                fprintf(fp,"%s,%c,%d",movie_chosen,s,num);
